...
export default function RSSetting({ value, onChange }: RSSettingProps) {
  ...
  const normalizedPeriods = useMemo(() => {
    if (periods.length === 1) {
      return [{ ...periods[0], ratio: 100, auto: true }];
    }
  ...
  const updatePeriod = (id: string, patch: Partial<RSPeriod>) => {
  ...
  return (
    <div className="p-4 flex flex-col gap-4 border-b">
      {normalizedPeriods.map((period, idx) => (
          ...
          <div className="w-full flex gap-2 items-center">
            ...
            <Popover>
              ...
              <PopoverContent className="w-auto p-0" align="start">
                <Calendar mode="range" selected={period.date} disabled={{ after: new Date() }} onSelect={(date) => updatePeriod(period.id, { date })} numberOfMonths={2} />
              </PopoverContent>
            </Popover>
